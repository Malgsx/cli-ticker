...
- `Updates Available` submenu with readable version changes, clickable update actions, and an `Update All` action. Updates run quietly in the background — no terminal windows — with progress shown in the menu, output logged to `updates.log`, and updated tools removed from the list automatically after the rescan that follows.
...
- `~/Library/Application Support/CLITicker/updates.log` (output of the most recent background update run, also reachable via `Open Report > Update Log`)
...
Recently Updated entries expire after 24 hours and are also written to the Markdown report. The 15-minute background scan still runs as a fallback.
